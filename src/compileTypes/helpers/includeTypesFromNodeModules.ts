...
import { getExposedModulePaths } from "./getExposedModulePaths";
...
export function includeTypesFromNodeModules(
  ...
  const exposedNpmPackages = Object.entries(federationConfig.exposes).flatMap(
    ([exposedModuleKey, expose]) => {
      return getExposedModulePaths(expose)
        .filter(
          (packagePath) =>
  ...
  try {
    ...
    logger.warn(
      "Typings was not included for npm package:",
      (err as Dict)?.url
    );
    logger.log(err);
  }
